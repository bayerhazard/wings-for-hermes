...
// Panel-head new chat button (same behavior as btnNewChat)
function handlePanelNewChat(){
  if(typeof btnNewChat==="function"){
    btnNewChat();
  } else {
    $("btnNewChat").click();
  }
...
function _normalizeAppearance(theme,skin){
  ...
  let nextSkin=_VALID_SKINS.has(rawSkin)?rawSkin:(legacy?legacy.skin:null);
  if(!nextSkin) nextSkin=nextTheme==='light'?'wings-light':'wings-dark';
...
function _pickTheme(name){
  const skin=name==='light'?'wings-light':name==='dark'?'wings-dark':window.matchMedia('(prefers-color-scheme:dark)').matches?'wings-dark':'wings-light';
  const appearance=_normalizeAppearance(name,skin);
...
function _pickSkin(name){
  _pickTheme(localStorage.getItem('wings-theme')||'dark');
}
...
(async()=>{
  ...
  try{
    ...
    window._showCliSessions=s.show_cli_sessions===undefined?(localStorage.getItem('wings-mode')==='advanced'):s.show_cli_sessions!==false;
