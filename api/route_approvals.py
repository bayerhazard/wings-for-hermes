...
def submit_gateway_pending_mirror(session_key: str, approval: dict) -> tuple[dict | None, int]:
    ...
    with _lock:
        ...
        if exact_local_entry is None and not run_id:
            ...
            # `request_id` is stamped once on the source entry and preserved
            # through the copy, so it uniquely reunites the notified copy with
            # its queued entry. Without this, the mirror is created with no
            # token, reconcile keeps the orphan, and
