/**
 * Wings for Hermes Service Worker
...
const CACHE_NAME = 'wings-for-hermes-shell-__WEBUI_VERSION__-w175';
...
const VQ = '?v=__WEBUI_VERSION__&w=175';
const SHELL_ASSETS = [
  './static/style.css' + VQ,
    ...
    event.respondWith(
      fetch(new Request(event.request, { cache: 'no-store' })).then((response) => {
        ...
        return caches.match('./').then((cached) => cached || new Response(
          ...
          '<p>Wings for Hermes requires a server connection. Please check your network and try again.</p>' +
          '</body></html>',
