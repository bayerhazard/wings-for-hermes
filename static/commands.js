...
async function cmdTheme(args){
  ...
  if(themes.includes(val)||legacyThemes.includes(val)){
    const appearance=_normalizeAppearance(val);
    localStorage.setItem('hermes-theme',appearance.theme);
    localStorage.removeItem('hermes-skin');
    _applyTheme(appearance.theme);
    try{await api('/api/settings',{method:'POST',body:JSON.stringify({theme:appearance.theme})});}catch(e){}
    const sel=$('settingsTheme');
    if(sel)sel.value=appearance.theme;
    if(typeof _syncThemePicker==='function') _syncThemePicker(appearance.theme);
    showToast(t('theme_set')+appearance.theme);
    return;
  }
  showToast(t('theme_usage')+themes.join('|')+(legacyThemes.length?' | legacy:'+legacyThemes.join('|'):''));
}
