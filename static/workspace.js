...
async function _collectOsDropUploads(dataTransfer) {
  ...
  if (items.length) {
    // Snapshot all entries synchronously before any async traversal: the
    // DataTransferItemList is invalidated by the first async tick, so entries
    // fetched during traversal can silently be null (#7142).
    const entries = [];
