...
async function startCompressionRecovery(btn){
  ...
  try{
    ...
    try{localStorage.setItem('wings-session',sid);}catch(_){}
...
function openWingsDashboard(event){
...
const MEDIA_PLAYBACK_STORAGE_KEY='wings-media-playback-rate';
...
const MODEL_STATE_KEY='wings-model-state';
const PENDING_SESSION_MODEL_PREFIX='wings-pending-session-model:';
...
function _readPersistedModelState(){
  ...
  const legacy=localStorage.getItem('wings-model');
...
function _writePersistedModelState(model, modelProvider){
  ...
  localStorage.setItem('wings-model', value);
  try{
...
function speakMessage(btn){
  ...
  // Extension-registered TTS engine (window.registerWingsTtsEngine). Synthesize
...
function autoReadLastAssistant(){
  ...
  const pref=localStorage.getItem('wings-tts-auto-read');
  if(pref!=='true') return;
  ...
  // Extension-registered TTS engine (window.registerWingsTtsEngine): synth via
...
const INFLIGHT_KEY = 'wings-inflight'; // localStorage key for in-flight session tracking
const INFLIGHT_STATE_KEY = 'wings-inflight-state'; // localStorage snapshots for mid-stream reload recovery
...
function _formatManualUpdateInstruction(info){
  ...
  return t('settings_update_manual_docker','docker pull ghcr.io/nesquena/wings-for-hermes:latest');
}
...
const WHATS_NEW_SUMMARY_STORAGE_KEY='wings-whats-new-generated-summaries';
...
function dismissUpdate(){
  ...
  sessionStorage.setItem('wings-update-dismissed','1');
}
...
const _activityDisclosureStoragePrefix='wings-activity-disclosure:';
...
function toggleWorkspaceHiddenFiles(value){
  ...
  try{localStorage.setItem('wings-workspace-show-hidden-files',S.showHiddenWorkspaceFiles?'1':'0');}catch(_){}
  _syncWorkspaceHiddenToggle();
  renderFileTree();
}
try{S.showHiddenWorkspaceFiles=localStorage.getItem('wings-workspace-show-hidden-files')==='1';}catch(_){}
