...
// Keep same-page settings merges FIFO so one response cannot race the next read-modify-write. (#6612)
...
async function loadSettingsPanel(){
  ...
  try{
    ...
    if(autoScrollFollowCb){
      ...
      // sync the global mirror too. Persist ONLY when the server explicitly
      // sent a boolean (never persist a synthesized default from an absent key).
...
function _applySavedSettingsUi(saved, body, opts){
  const {sendKey,showTokenUsage,showQuotaChip,showConversationOutline,showBusyPlaceholderHint,showTps,fadeTextEffect,showCliSessions,theme,language,sidebarDensity,fontSize}=opts;
