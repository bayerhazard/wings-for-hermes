...
def test_primary_composer_stop_renders_localized_error_and_preserves_success():
    ...
    german = re.search(
        rf"{cancel_key}:\s*'((?:\\'|[^'])*)'", I18N_JS[I18N_JS.index("de:"):]
    )
    assert english and german
    english_message = english.group(1).replace("\\'", "'")
    german_message = german.group(1).replace("\\'", "'")
    script = (r'''
...
for (const [message, result] of [[__ENGLISH__, false], [__GERMAN__, false], ['unused', true]]) {
...
'''.replace("__SHOW_TOAST_SRC__", SHOW_TOAST_SRC)
    ...
    assert result["renders"] == [
        ...
        {"message": german_message, "className": "toast show error", "duration": 20000, "copy": True},
    ]
