...
function setUIMode(mode) {
  ...
  if (getUIMode() === 'basic') {
    ...
    if (ADVANCED_SETTINGS_SECTIONS.has(_currentSettingsSection)) switchSettingsSection('system');
  }
}
...
let _settingsSection = 'system';
let _currentSettingsSection = 'system';
...
function switchSettingsSection(name,opts){
  ...
  if(getUIMode()!=='advanced' && ADVANCED_SETTINGS_SECTIONS.has(section)) section='system';
...
function _appearancePayloadFromUi(){
  const theme=($('settingsTheme')||{}).value || localStorage.getItem('hermes-theme') || 'dark';
  const chatActivityModeSel=$('settingsChatActivityDisplayMode');
  return {
    theme: theme,
    skin: theme==='light'?'wings-light':theme==='system'
      ? (window.matchMedia('(prefers-color-scheme:dark)').matches?'wings-dark':'wings-light')
      : 'wings-dark',
    ...
    activity_feed_expanded_default: false,
...
function _rememberAppearanceSaved(payload){
  ...
  _settingsSkinOnOpen=payload.skin||(_settingsThemeOnOpen==='light'?'wings-light':_settingsThemeOnOpen==='system'?(window.matchMedia('(prefers-color-scheme:dark)').matches?'wings-dark':'wings-light'):'wings-dark');
...
function _preferencesPayloadFromUi(){
  ...
  payload.show_token_usage=false;
  payload.show_quota_chip=false;
  payload.api_redact_enabled=false;
  payload.sidebar_density='compact';
  const isAdvanced=window._uiMode==='advanced';
  payload.show_cli_sessions=isAdvanced;
  payload.show_claude_code_sessions=isAdvanced;
  payload.show_cron_sessions=isAdvanced;
  payload.show_webhook_sessions=isAdvanced;
  payload.show_previous_messaging_sessions=isAdvanced;
  payload.show_busy_placeholder_hint=false;
  payload.new_chat_on_workspace_switch=false;
  payload.rtl=false;
...
function _speechPreferencesPayloadFromUi(){
  ...
  _setOwnedSpeechPayload(payload,'tts_auto_read',false);
...
async function loadSettingsPanel(){
  ...
  try{
    ...
    window._showTitlebarProfile=false;
    ...
    window._apiRedactEnabled=false;
    const isAdvanced=window._uiMode==='advanced';
    window._showCliSessions=isAdvanced;
    window._showCronSessions=isAdvanced;
    window._showClaudeCodeSessions=isAdvanced;
    window._showWebhookSessions=isAdvanced;
    window._showPreviousMessagingSessions=isAdvanced;
    const syncCb=$('settingsSyncInsights');
    ...
    try{localStorage.setItem('hermes-rtl','false');document.documentElement.classList.remove('chat-content-rtl');}catch(_){}
    window._rtl=false;
    ...
    window._newChatOnWorkspaceSwitch=false;
    if(typeof _applyBusyComposerPlaceholder==='function') _applyBusyComposerPlaceholder();
...
async function saveSettings(andClose){
  ...
  const skin=theme==='light'?'wings-light':theme==='system'?(window.matchMedia('(prefers-color-scheme:dark)').matches?'wings-dark':'wings-light'):'wings-dark';
  ...
  body.session_endless_scroll=true;
  ...
  body.show_quota_chip=false;
  body.show_conversation_outline=showConversationOutline===true;
  body.show_busy_placeholder_hint=false;
  ...
  body.api_redact_enabled=false;
  const isAdvanced=window._uiMode==='advanced';
  body.show_cli_sessions=isAdvanced;
  body.show_claude_code_sessions=isAdvanced;
  body.show_cron_sessions=isAdvanced;
  body.show_webhook_sessions=isAdvanced;
  body.show_previous_messaging_sessions=isAdvanced;
  ...
  if(pw && pw.trim()){
    ...
    try{
      ...
      _applySavedSettingsUi(saved, body, {sendKey,showTokenUsage:false,showQuotaChip:false,showConversationOutline,showBusyPlaceholderHint:false,showTps,fadeTextEffect,showCliSessions:isAdvanced,theme,skin,language,sidebarDensity:'compact',fontSize});
