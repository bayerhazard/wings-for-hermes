...
wings_version: 26.9.5            # wird bei jedem Release aktualisiert
...
| 6e | 2026-09-16 | — (Wings-only) | Brand-Lockup im Chatfenster | Wortmarken-Bild (472×167 „AImighty") + Unterzeile „Wings for Hermes" ersetzt durch `wings-shield.svg` (dein 150×150-Schild, gold + weiße AI) **und** das Wort „Wings" daneben; ein Asset für beide Themes (weiß auf Gold). Schriftgröße **abgeleitet, nicht geschätzt**: AI-Kappe = 59,599/150 = 39,73 %, Geist-Kappe = 0,71 em → `font-size = .56 × Markenhöhe` (= 44,8px bei 80px), Gewicht 300. Mobil ≤640px gestapelt/zentriert, gleiche Größe. Dazu der Testfix in `test_issue856_pinned_indicator_layout.py` (hielt `header.ondblclick` fest) und `tests/test_wings_brand.py` (5 Assertions). Hinweis: der SVG-Kommentar darf keine doppelten Bindestriche enthalten — sonst ist das SVG kein gültiges XML und `<img>` rendert lautlos nichts |
